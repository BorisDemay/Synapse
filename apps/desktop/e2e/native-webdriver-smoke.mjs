...
try {
  ...
  // A restart can restore the remembered session (ADR 0014) and reach the vault
  // without ever showing the passphrase form. Wait for either state and only
  // unlock when the form is actually present.
  async function waitUnlockOrVault() {
    return driver.wait(async () => {
      if (
        (await driver.findElements(By.css('input[type="password"]'))).length ===
        1
      )
        return "unlock";
      if (
  ...
  async function unlockIfNeeded() {
    ...
    await fields[0].sendKeys(phrase);
