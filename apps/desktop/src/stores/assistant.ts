...
function isConversationMessage(
  value: unknown,
): value is AssistantConversationMessage {
...
export const useAssistantStore = defineStore("assistant", () => {
  ...
  async function restoreConversations(): Promise<void> {
    try {
      const stored = await useVaultStore().loadAssistantConversations();
      const snapshot = stored ? parseConversationSnapshot(stored) : undefined;
  ...
  async function executeToolCall(call: CodexFunctionCall): Promise<{
    ...
    if (call.name === "create_note" && noteId === undefined) {
      const createdNoteId = vault.nextNotePath(markdown);
