...
fn fixture_encrypted_operation() -> EncryptedPushOperation {
    EncryptedPushOperation {
        ...
        base_revision: 4,
        ciphertext: vec![212, 91, 3, 214, 42],
        nonce: vec![17; 24],
        aad_version: 1,
...
fn sync_contracts_round_trip_without_unknown_fields() {
    ...
    let request = PullRequest {
        ...
        cursor: Some(SyncCursor::new("opaque-cursor-5").expect("valid cursor")),
        limit: 100,
    };
    let response = PullResponse {
        protocol_version: 1,
        operations: vec![operation.clone()],
        next_cursor: Some(SyncCursor::new("opaque-cursor-6").expect("valid cursor")),
    };
