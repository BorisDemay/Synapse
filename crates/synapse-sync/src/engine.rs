    ...
    S: RetryScheduler,
{
    ...
    pub fn synchronize_with_pulled(
        ...
        // Refresh the durable remote head before constructing or delivering
        // queued mutations. This keeps base_revision current and lets the
        // client surface a conflict instead of blindly pushing stale state.
        let (pull_state, pulled) = self.pull_until_current_cursor()?;
        if pull_state != SyncState::Synced {
            return Ok((pull_state, pulled));
        }
