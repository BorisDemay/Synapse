...
Sur le NAS, les artefacts versionnés sont copiés avant le changement de stack.
`latest.json` et `web.json` sont remplacés atomiquement en dernier, uniquement
après `/health/version` et la disponibilité des artefacts. Une erreur restaure
la version d’image précédente sans publier les nouveaux manifestes.
