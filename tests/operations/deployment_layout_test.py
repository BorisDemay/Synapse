...
class DeploymentLayoutTests(unittest.TestCase):
    ...
    def test_accepts_server_image_update_with_two_compose_paths(self):
    ...
    def test_rejects_invalid_compose_without_echoing_configuration(self):
        ...
        result = self.run_checker(current, invalid)
    ...
    def test_rejects_required_server_image_missing(self):
        ...
        result = self.run_checker(current, self.write_compose('missing-server-image.yml', candidate))
    ...
    def test_rejects_postgres_image_or_storage_layout_change(self):
        ...
        for name, change in (
            ...
            with self.subTest(name=name):
                candidate = compatible_compose()
                change(candidate)
                result = self.run_checker(current, self.write_compose(name, candidate))
                self.assertNotEqual(result.returncode, 0)
