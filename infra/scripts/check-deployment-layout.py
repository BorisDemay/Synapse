...
def render_layout(env_file: str, compose_file: str, project: str) -> dict[str, Any]:
    try:
...
def main(arguments: list[str]) -> int:
    ...
    # Compose derives the project name from the compose file's directory, so a
    # candidate rendered from the staging directory would disagree with the
    # running compose even when the persistent layout is identical. Pin one
    # project name taken from the current deployment for both renders.
    project = os.path.basename(os.path.dirname(os.path.abspath(arguments[1])))

    try:
        current = render_layout(arguments[0], arguments[1], project)
        candidate = render_layout(arguments[0], arguments[2], project)
